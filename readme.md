...
- El código de ejemplo se encuentra [aquí](https://github.com/sebastiantorres86/node/tree/master/ejemplos/05-expressAPI)
...
app.get('/', function(request, response) {
  response.sendFile(path.join(__dirname, 'indes.html'))
})
...
- Por medio del método `join` podemos concatener la ruta del directorio (\_\_dirname`) y el nombre de nuestro archivo HTML
...
- Podemos ver que es bastante similar pero le agrega un par de parámetros más
  - El primer parámetro es el error, de esta forma podemos saber que pasó
...
- Gracias a esta arquitectura podemos agregarle funcionalidad a nuestro servidor de express ya sea creando nuestros propios middlewares o usando algunos ya existentes
- Express tiene una gran cantidad de middlewares que se pueden configurar
...
const miMiddleware = function(req, res, next) {
  console.log('paso por un middleware')
  next()
}
...
Imaginemos que tenemos que desarrollar una aplicación web de un videoclub. La página web, la interfaz que va a ver el usuario, la vamos a desarrollar con HTML y CSS para que se vea bien, y con Javascript vamos a poder agregarle animaciones e interacción.
...
6. Una vez realizado eso, empezamos con el código:
...
// estas dos líneas permiten que cuando desde una web nos envien un objeto de JS, express
// lo lea e interprete automáticamente y nos lo guarde en la propiedad req.body
...
// creo un array, donde voy a ir almacenando las películas
// de esta forma la información no está escrita en la web, sino que la guardamos en un servidor, en en lugar central
...
// creo una ruta que devuelve el array de películas
router.get('/api/movies', (req, res) => {
  // respondo al cliente con el array de películas
  res.json(movies)
})

// exporto mi módulo router
module.exports = router
```
...
```
// creamos una función que pasándole una película como parámetro, nos retorne el HTML de esa película
...
// una función que busca el contenedor de películas y adjunta una nueva película
...
fetch('http://localhost:3000/api/movies')
  ...
  .then(function (movies)) {
    // el parámetro respuesta es lo que nos contesta el servidor (en este caso el array de las películas)

    // por cada película, llamamos a la función que creamos arriba para ir agregándola en el DOM
...
- Entonces, no importa si accedemos desde una computadora, un celular, o cualquier otro dispositivo: como siempre estamos usando la misma API, vamos a obtener los mismo datos
...
router.get('/api/movies/:id', (req, res) => {
  ...
  // si no encuentra ninguna película, le devolvemos un estado 404 not found y un mensaje
  if (!movie) {
    return res.status(404).send(`La película con id ${id} no existe.`)
  }

  // respondemos a quien nos pidió al película con el objeto json que encontramos
  res.json(movie)
})
```

- Si navegamos a la ruta: `http://localhost:3000/api/movies/1` ya podemos ver en el browser el JSON del objeto específico
  - Podemos probar las otras url cambiando 1 por 2, 3 y ver que pasa si pasamos parámetro donde no tenemos valores en el array (por ejemplo 15)
...
const configureEvents = () => {
  // configuro el botón para abrir el modal con la información de la película
  const triggersEdit = document.querySelectorAll(".trigger-view")
  treiggersEdit.forEach(trigger = trigger.addEventListener("click", oopenModal))
}

// esta función la ejecutamos cuando hacemos click en el botón para ver el detalle de una película
const openModal = e => {
  ...
  fetch(`http://localhost:3000/api/movies/${movieId}`)
    .then(res => res.json())
    .then(movie => {
      // modifica todos los elementos del modal con la información de la película
...
const closeButton = document.querySelecctor(".close-button")
...
- Una vez que la API nos responde, modifica el HTML y mostramos el modal llamando a la función `toggleModal()`
...
const saveMovie = () => {
  ...
  // creamos el objeto que estamos creando y le vamos a enviar el servidor para que guarde
  const newMovie = {
    title: title,
    summary: summary,
    year: year
  ...
  //finalmente, con ajax, le enviamos un HTTP POST al servidor para que guarde el objeto y le pasamos como data el nuevo objeto
  fetch('http://localhost:3000/api/movies', {
    method: 'post'
    headers: {
...
router.post('/api/movies', (req, res) => {
  ...
  // para eso, lo que hacemos es buscar la última película, y le sumamos 1 a ese ID
  let nextId = 1
  if (movies.length > 0) {
